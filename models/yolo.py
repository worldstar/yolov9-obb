...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        super().__init__()
        #print("Anchors inside Model __init__:", anchors, type(anchors))
        self.nc = nc  # number of classes
        ...
        #print("Anchors inside Model __init__:", self.anchors, type(self.anchors))
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov9.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        super().__init__()
        ...
        if anchors:
            if isinstance(anchors, list) and all(isinstance(a, list) for a in anchors):
                LOGGER.info(f'Overriding model.yaml anchors with anchors={anchors}')
                self.yaml['anchors'] = round(anchors)  # override yaml value
                #self.yaml['anchors'] = anchors  # Correctly override yaml value as a list of lists 
            else:
                raise TypeError("Expected anchors to be a list of lists, not an integer.")
        ...
        if isinstance(m, Detect):
            ...
            #if m.anchors.shape[0] != m.stride.numel():
            #    m.stride = torch.tensor([8, 16, 32]).to(m.anchors.device)  # Ensure this matches the scales for your anchors
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        n = n_ = max(round(n * gd), 1) if n > 1 else n  # depth gain
        if m in [Conv, AConv, Bottleneck, SPP, SPPF, DWConv, MixConv2d, BottleneckCSP, nn.ConvTranspose2d, DWConvTranspose2d, SPPCSPC, ADown,
            ELAN1, RepNCSPELAN4, SPPELAN]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)
